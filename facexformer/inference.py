...
def test(args):
    ...
    if tasks[0] == 0: #maybe use preds for KL divergence between source and result?
...
def get_attribute(image, model, device): #for optimizing h.

    if isinstance(image, torch.Tensor):
        # For tensor input, we need to resize and normalize only
        
        # Make sure the tensor is in format [C, H, W]
        if len(image.shape) == 4:  # [B, C, H, W]
            image = image.squeeze(0)  # Remove batch dimension
        
        # Resize to 224x224 using functional API to preserve gradients
        image = torchvision.transforms.functional.resize(
            image, size=[224, 224], interpolation=InterpolationMode.BICUBIC
        )
        ...
        # For PIL Image or numpy array, use the original transformation pipeline
        transforms_image = torchvision.transforms.Compose([
            torchvision.transforms.Resize(size=(224,224), interpolation=InterpolationMode.BICUBIC),
            torchvision.transforms.ToTensor(),
            torchvision.transforms.Normalize(mean=[0.485, 0.456, 0.406], std=[0.229, 0.224, 0.225]),
    ...
    _, _, attribute_output, _, _, _, _, _  = model(images, labels, tasks)
    attribute_probability = torch.sigmoid(attribute_output)
    #print(f'attribute output: {attribute_output}')
    
    data = {'image': image, 'label': {"segmentation":torch.zeros([224,224]), "lnm_seg": torch.zeros([5, 2]),"landmark": torch.zeros([68, 2]), "headpose": torch.zeros([3]), "attribute": torch.zeros([40]), "a_g_e": torch.zeros([3]), 'visibility': torch.zeros([29])}, 'task': torch.tensor([0])}
    images, labels, tasks = data["image"], data["label"], data["task"]
    images = images.unsqueeze(0).to(device=device)
    for k in labels.keys():
        labels[k] = labels[k].unsqueeze(0).to(device=device)
    ...
    _, _, _, _, _, _, _, seg_output  = model(images, labels, tasks)
    preds = seg_output.softmax(dim=1)
    #index: 1 is for face skin, 2 for eyebrows, 4 and 5 for eyes, 6 for nose, 7 and 9 for mouth
