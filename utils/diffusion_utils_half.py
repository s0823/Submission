...
def extract(a, t, x_shape):
    ...
    bs, = t.shape
    assert x_shape[0] == bs
    # Create tensor with same dtype as t for consistency
    a_tensor = torch.tensor(a, dtype=t.dtype, device=t.device)
...
def denoising_step(xt, t, t_next, *,
    ...
    # Ensure all tensors have consistent precision
    working_dtype = torch.float16  # Use the input tensor's dtype for consistency
    
    # Convert t and t_next to the same precision as xt if needed
    xt=xt.to(dtype=working_dtype)
    t = t.to(dtype=working_dtype)
    t_next = t_next.to(dtype=working_dtype)
    
    # Convert edit_h to the same precision if provided
    if edit_h is not None:
        edit_h = edit_h.to(dtype=working_dtype)
    ...
    if sampling_type == 'ddpm':
        ...
        xt_next = xt_next.to(dtype=working_dtype)
    ...
    # Ensure consistent dtype for outputs
    xt_next = xt_next.to(dtype=working_dtype)
    # Handle mid_h for output
    if 'mid_h' in locals():
        mid_h = mid_h.to(dtype=working_dtype)
    else:
        mid_h = None

    if out_x0_t == True and 'x0_t' in locals():
        x0_t = x0_t.to(dtype=working_dtype)
